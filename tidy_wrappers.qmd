...
qbench <- function(mb_result){
   # extract the median time in seconds
   mb_result |> 
      group_by(expr) |>
      summarize(seconds = median(time)/1e9)
   
}
qbench(microbenchmark(do_base(tidy_df[1:1e7,]),do_dplyr(tidy_df[1:1e7,]),times = 3,unit = "seconds"))
